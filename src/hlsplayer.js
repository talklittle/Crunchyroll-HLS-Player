...
var skipOp =false,skipEd = false,resume =true,autoStart = true,adsEnabled=true,isPremium = false;
var player,vastAd;
var rplayhead=0;
$(function(){  
  ...
  $('#showmedia_video').prepend('<video id="video" controls class="video-js vjs-default-skin" width="960" height="570"></video>');
  ...
  var mediaId = temp[temp.length-1];
  play(mediaId,quality);
  ...
  chrome.runtime.sendMessage({text:"getMediaUrl",mediaId:mediaId,quality:quality},function(reply){
	...
	player.addCuepoint({
	  ...
	  onStart: function(params){
		...
		if(skipOp==="true"&&isNaN(opEnd)===false)
	      player.currentTime(opEnd);
	  },
	  onEnd: function(params){
	    //console.log("Start of ED");
		if(skipOp==="true"&&isNaN(edStart)===false)
	      player.currentTime(edStart+89);
	  },
	  params:{error:false}
	});}
    if(resume === "true")
	  player.currentTime(reply.playhead);
    if(autoStart === "true")
	    player.play();
